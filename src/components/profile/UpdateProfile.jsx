...
const UpdateProfile = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                if (user) {
                    const response = await userData(user.id); // Replace with actual API call
    ...
    const validate = () => {
        const newErrors = {};
        if (!profileData.fullName.trim() || profileData.fullName.trim().split(" ").length < 2) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (selectedFile) {
                const uploadResponse = await uploadImage(selectedFile, 'user_profiles', user.id);
                imageUrl = uploadResponse.secure_url;
            }

            // Ensure dateOfBirth is in correct format before sending to server
            const formattedBirthDate = profileData.dateOfBirth ? new Date(profileData.dateOfBirth).toISOString() : null;
