    ...
    return (
        <div className="flex justify-center my-4">
            {steps.map((step, index) => (
                ...
                ${index <= currentStep ? "bg-blue-600" : "bg-gray-700 "} ${index === currentStep ? "ring-2 ring-blue-400" : ""}`}
                    >
                        <span className={`${index <= currentStep ? "text-white" : "text-gray-500"}`}>{step.icon}</span>
                    </div>
                    <span className={`mx-2 text-sm ${index <= currentStep ? "text-white" : "text-gray-500"}`}>
                        {step.label}
                    </span>
                    {index < steps.length - 1 && (
                        <div className={`w-8 md:w-20 h-1 mr-4 ${index < currentStep ? "bg-blue-600" : "bg-gray-700"}`} />
                    )}
                </div>
            ))}
        </div>
